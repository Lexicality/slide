...
if (CLIENT) then
	...
	killicon.AddFont(
		"weapon_real_cs_knife", "CSKillIcons", SWEP.IconLetter, Color(255, 80, 0, 255)
	)
...
function SWEP:PrimaryAttack()
	...
	if (trace.Hit) then
		...
		if trace.Entity:IsPlayer() or string.find(trace.Entity:GetClass(), "npc") or
			string.find(trace.Entity:GetClass(), "prop_ragdoll") then
			...
			self.Weapon:EmitSound(
				"weapons/knife/knife_hit" .. math.random(1, 4) .. ".wav"
			)
		else
			self.Weapon:SendWeaponAnim(ACT_VM_HITCENTER)
			local bullet = {}
