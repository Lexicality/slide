...
function SWEP:Think()
	...
	elseif self.Primed == 1 and not self.Owner:KeyDown(IN_ATTACK2) and
		not self.PrimaryThrow then
		if self.Throw < CurTime() then
			self.Primed = 2
			self.Throw = CurTime() + 1.5
...
function SWEP:ThrowFar()
	...
	ent:SetAngles(
		Vector(math.random(1, 100), math.random(1, 100), math.random(1, 100))
	)
	...
	phys:ApplyForceCenter(
		self.Owner:GetAimVector() * self.Force * 1.2 + Vector(0, 0, 200)
	)
	phys:AddAngleVelocity(
		Vector(math.random(-500, 500), math.random(-500, 500), math.random(-500, 500))
	)
...
function SWEP:ThrowShort()
	...
	phys:ApplyForceCenter(
		self.Owner:GetAimVector() * self.Force * 2 + Vector(0, 0, 0)
	)
	phys:AddAngleVelocity(
		Vector(math.random(-500, 500), math.random(-500, 500), math.random(-500, 500))
	)
...
function SWEP:PrimaryAttack()
	...
	if self.Throw < CurTime() and self.Primed == 0 and
