...
if (CLIENT) then
	...
	function FlashEffect()
		if LocalPlayer():GetNetworkedFloat("FLASHED_END") > CurTime() then
			...
			if (FlashedEnd - CurTime() > FLASHTIMER) then
				...
				local FlashAlpha = 1 - (CurTime() - (FlashedEnd - FLASHTIMER)) /
                   					(FlashedEnd - (FlashedEnd - FLASHTIMER));
	...
	local function StunEffect()
		...
		if (FlashedEnd > CurTime() and FlashedEnd - EFFECT_DELAY - CurTime() <=
			FLASHTIMER) then
			local FlashAlpha = 1 - (CurTime() - (FlashedEnd - FLASHTIMER)) / (FLASHTIMER);
			DrawMotionBlur(
				0, FlashAlpha / ((FLASHTIMER + EFFECT_DELAY) / (FLASHTIMER * 4)), 0
			);
